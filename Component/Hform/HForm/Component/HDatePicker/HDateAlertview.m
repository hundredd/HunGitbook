...
@implementation HDateAlertview
{
    UIView *view;
    int tag;
}
@synthesize title,message,cancelbtn,surebtn,centerBtn;
...
{
    [self initwithtitle:str andmessage:str1 andcancelbtn:cancel andotherbtn:other andCenterBtn:NO];
}
...
-(void)initwithtitle:(NSString *)str andmessage:(NSString *)str1 andcancelbtn:(NSString *)cancel andotherbtn:(NSString *)other andCenterBtn:(BOOL)showCenter
{
    ...
    surebtn.tag = 2;
    [cancelbtn setBackgroundColor:hLightFontColor];
    [surebtn setBackgroundColor:HNomalBlueBackColor];
    ...
    if (showCenter)
    {
        ...
        [btn setTitle:@"长期" forState:0];
        btn.tag = 3;
        [btn setBackgroundColor:HNomalBlueBackColor];
        centerBtn = btn;
        //进行layer的布局
    ...
    {
        if (showCenter) {
            cancelbtn.frame = CGRectMake(10, self.frame.size.height-45, self.frame.size.width/3-30, 35);
            [self addSubview:cancelbtn];
            
            centerBtn.frame = CGRectMake(self.frame.size.width/3+10, self.frame.size.height-45, self.frame.size.width/3-30, 35);
            [self addSubview:centerBtn];
            
            surebtn.frame = CGRectMake(self.frame.size.width/3*2+10, self.frame.size.height-45, self.frame.size.width/3-30, 35);
            [self addSubview:surebtn];
...
-(void)selectbtn:(id)sender
{
    ...
    if (self.delegate&&[self.delegate respondsToSelector:@selector(btnindex::)] )
    {
